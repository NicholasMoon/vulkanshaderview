...
// create vulkan image
void Image::createImage(uint32_t mipLevels, VkImageCreateInfo& imageInfo, LogicalDevice& logicaldevice, PhysicalDevice &physicaldevice) {
    // create vulkan image to store texture in
    if (vkCreateImage(logicaldevice.device, &imageInfo, nullptr, &vkImage) != VK_SUCCESS) {
        throw std::runtime_error("failed to create image!");
    }

    VkMemoryRequirements memRequirements;
    vkGetImageMemoryRequirements(logicaldevice.device, vkImage, &memRequirements);
...
void Image::copyBufferToImage(VkBuffer buffer, CommandPool& commandpool, LogicalDevice &logicaldevice, uint32_t layerCount) {
...
void Image::transitionImageLayout(VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels, CommandPool& commandpool, LogicalDevice &logicaldevice, uint32_t layerCount) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands(commandpool, logicaldevice);
...
VkImageView Image::createImageView(VkImageViewCreateInfo& viewInfo, LogicalDevice& logicaldevice) {
