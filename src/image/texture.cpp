...
void Texture::createTextureImage(std::string texturepath, LogicalDevice& logicaldevice, PhysicalDevice& physicaldevice, CommandPool& commandpool) {
    ...
    createImage(mipLevels, imageInfo, logicaldevice, physicaldevice);

    // transition the texture image to VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL
    transitionImageLayout(VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipLevels, commandpool, logicaldevice, 1);

    // execute the buffer to image copy operation
    copyBufferToImage(stagingBuffer.vkBuffer, commandpool, logicaldevice, 1);
