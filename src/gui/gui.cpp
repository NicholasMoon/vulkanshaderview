...
IMGUI_API void GUI::showGUI(bool* p_open) {
    ...
    {
        // Camera
        ImGui::SliderFloat("camera eye X", &camera_ptr_imgui->m_eye.x, -15.0f, 15.0f, "%.3f");
        ImGui::SliderFloat("camera eye Y", &camera_ptr_imgui->m_eye.y, -15.0f, 15.0f, "%.3f");
        ImGui::SliderFloat("camera eye Z", &camera_ptr_imgui->m_eye.z, -15.0f, 15.0f, "%.3f");
    ...
    if (ImGui::CollapsingHeader("Material"))
    {
        ...
        for (int i = 0; i < scene_ptr_imgui->size(); ++i) {
            if ((*scene_ptr_imgui)[i]->m_light == nullptr) {
                ...
                ImGui::SliderFloat(label_metallic.c_str(), &((*scene_ptr_imgui)[i]->m_ubo_per_primitive.metallic), 0.0f, 1.0f, "%.3f");
                ImGui::SliderFloat(label_roughness.c_str(), &((*scene_ptr_imgui)[i]->m_ubo_per_primitive.roughness), 0.0f, 1.0f, "%.3f");
            }
        }
    }
