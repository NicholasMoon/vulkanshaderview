...
bool MyVK::checkDeviceExtensionSupport(VkPhysicalDevice device) {
...
bool MyVK::isDeviceSuitable(VkPhysicalDevice device) {
...
VkSampleCountFlagBits MyVK::getMaxUsableSampleCount() {
...
MyVK::QueueFamilyIndices MyVK::findQueueFamilies(VkPhysicalDevice device) {
...
MyVK::SwapChainSupportDetails MyVK::querySwapChainSupport(VkPhysicalDevice device) {
...
VkSurfaceFormatKHR MyVK::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
...
VkPresentModeKHR MyVK::chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) {
...
VkExtent2D MyVK::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
...
VkImageView MyVK::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels) {
...
void MyVK::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
void MyVK::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
// create vulkan image
void MyVK::createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
...
void MyVK::generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels) {
...
VkFormat MyVK::findSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features) {
...
bool MyVK::hasStencilComponent(VkFormat format) {
    return format == VK_FORMAT_D32_SFLOAT_S8_UINT || format == VK_FORMAT_D24_UNORM_S8_UINT;
}

// create vulkan structures related to depth buffer
void MyVK::createDepthResources() {
...
void MyVK::createColorResources() {
